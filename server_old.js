...
const { admin } = require('./core/database/init/schema');
...
async function bootstrap() {
    ...
    const database = require('./core/database/init');
    console.log('✅ База данных подключена');
    const { connection } = await database.initDatabase(config.database);
    // Импортируем репозитории
    const { AdminRepository, UserRepository, MessageRepository } = require('./core/repositories');
    const repositories = {
    ...
    const { AdminService, AuthService, ChatService, UserService, SettingsService, SocketAuthService} = require('./core/services');
    const services = {
        authService: new AuthService(repositories, config.jwtSreect || 'default-secret-key'),
    ...
    const { AuthController, AdminController } = require('./core/controllers');
    const controllers = {
    ...
    const {SocketAuthMiddleware } = require('./core/middleware');
    const middleware = {
